% Toy experiment on the two-moon synthetic data.
% Feiping Nie, Xiaoqian Wang, Michael I. Jordan, Heng Huang.
% The Constrained Laplacian Rank Algorithm for Graph-Based Clustering.
% The Thirtieth AAAI Conference on Artificial Intelligence (AAAI-16)

clc;  close all;
currentFolder = pwd;
addpath(genpath(currentFolder));

newdata = 1;
datatype = 2; % 1: three-Gaussian data, 2: three-ring data

if newdata == 1
    clearvars -except datatype;
    
    if datatype == 1
        [X, y, n1, n2, n3] = threegaussian_dim_gen(300, 0.05, 10, 0.5); c = 3;
    else
        [X, y] = three_ring_dim_gen; c = 3;
    end;
end;
XX=[X,y'];
la = y;
% The original data
figure; 
plot(X(:,1),X(:,2),'.k', 'MarkerSize', 18); hold on;
plot(X(la==1,1),X(la==1,2),'.r', 'MarkerSize', 18); hold on;
plot(X(la==2,1),X(la==2,2),'.', 'MarkerSize', 18); hold on;
axis equal;

%[la, A, evs] = CAN(X', c,15,1);
A0 = constructW_PKN(X', 10, 0);
A = A0;
A = (A+A')/2;
figure('name','Learned graph by PNN'); 
imshow(A,[]); colormap jet; colorbar;

[affinity_matrix] = buildGraph(X, 10);
figure('name','Learned graph by KNN'); 
imshow(affinity_matrix,[]); colormap jet; colorbar;

A0 = construcW_CAN(X, 10, 0);
A = A0;
A = (A+A')/2;
figure('name','Learned graph by CAN'); 
imshow(A,[]); colormap jet; colorbar;


% Probabilistic neighbors, line width denotes similarity
figure; 
plot(X(:,1),X(:,2),'.k', 'MarkerSize', 18); hold on;
plot(X(la==1,1),X(la==1,2),'.r', 'MarkerSize', 18); hold on;
plot(X(la==2,1),X(la==2,2),'.', 'MarkerSize', 18); hold on;
nn = 2*100;
for ii = 1 : nn;
    for jj = 1 : ii
        weight = A(ii, jj);
        if weight > 0
            plot([X(ii, 1), X(jj, 1)], [X(ii, 2), X(jj, 2)], '-g', 'LineWidth', 15*weight), hold on;
        end
    end;
end;
axis equal;

% [y, S, evs, cs] = CLR(A0, c);
% A = (S+S')/2;
% la = y;
% % Adaptive neighbors with L2, line width denotes similarity
% figure; 
% plot(X(:,1),X(:,2),'.k', 'MarkerSize', 18); hold on;
% plot(X(la==1,1),X(la==1,2),'.r', 'MarkerSize', 18); hold on;
% plot(X(la==2,1),X(la==2,2),'.', 'MarkerSize', 18); hold on;
% nn = 2*num0;
% for ii = 1 : nn;
%     for jj = 1 : ii
%         weight = A(ii, jj);
%         if weight > 0
%             plot([X(ii, 1), X(jj, 1)], [X(ii, 2), X(jj, 2)], '-g', 'LineWidth', 15*weight), hold on;
%         end
%     end;
% end;
% axis equal;
